...
const ColorPalette = () => {
  ...
  const moods = ["Energetic", "Elegant", "Bold", "Minimal", "Warm", "Cool"];
  ...
  const processApiResponse = (apiResponse) => {
        ...
        processedColors = colorsData
          .map((color, index) => {
            if (typeof color === "string") {
              ...
              const hsl =
                color.hsl || (rgb ? rgbToHsl(rgb.r, rgb.g, rgb.b) : null);
    ...
    processedColors = processedColors.filter((c) => {
      ...
      return (
        n !== "background" && n !== "text" && k !== "background" && k !== "text"
      );
    });
    ...
    const getAccessibilityInfo = (data) => {
      ...
      return {
        ...
        notes:
          acc.notes || acc.description || "Accessibility compliance verified",
      };
    };
    ...
    const getGradients = (data, colors) => {
      ...
      const createGradient = (from, to, deg = 135) => {
        const fromColor =
          typeof from === "string" ? from : from?.hex || colors[0];
        const toColor =
          typeof to === "string" ? to : to?.hex || colors[1] || colors[0];
      ...
      const safeLinear = (primary, secondary, deg = 135) => {
        const a =
          typeof primary === "string" ? primary : primary?.hex || colors[0];
        const b =
          typeof secondary === "string"
            ? secondary
            : secondary?.hex || colors[1] || colors[0];
        ...
        return `linear-gradient(${deg}deg, ${tint(a, 0.1)} 0%, ${shade(
          b,
          0.1
        )} 100%)`;
      };
      ...
      const neutralLinear = `linear-gradient(180deg, #FFFFFF 0%, ${tint(
        neutralEnd,
        0.8
      )} 100%)`;

      // Complementary: de-intensify by tinting both ends and change angle
      const compLinear = safeLinear(
        tint(primaryB, 0.15),
        tint(accentA, 0.15),
        120
      );
      ...
      return {
        primary: gradients.primary || {
          linear: safeLinear(primaryA, primaryB),
        },
    ...
    const palette = {
      ...
      name:
        paletteObject?.name || paletteObject?.title || "AI Generated Palette",
      description:
        paletteObject?.description ||
        paletteObject?.summary ||
        "Auto-generated by AI based on your inputs",
      colors: processedColors,
      hexArray: hexArray,
      gradients: getGradients(paletteObject, hexArray),
      interactiveStates:
        paletteObject?.interactiveStates || paletteObject?.states || {},
      ...
      paletteType:
        paletteObject?.paletteType || paletteObject?.type || "custom",
      colorHarmony:
        paletteObject?.colorHarmony || paletteObject?.harmony || "balanced",
      accessibility: getAccessibilityInfo(paletteObject),
      tags: paletteObject?.tags || [
        formData.industry || "General",
        formData.mood || "Professional",
      ],
      metadata: {
  ...
  const generatePalette = async () => {
    ...
    try {
      ...
      const baseLines = [
        `Generate a professional color palette for ${
          formData.industry || "general"
        } industry.`,
      ...
      if (formData.baseColor) {
        baseLines.push(
          `Use ${formData.baseColor} as the primary color and create complementary colors.`
        );
      }
      baseLines.push(
        "Ensure all colors are unique and include primary, secondary, accent, and neutral colors."
      );
      ...
      try {
        ...
        console.warn(
          `Primary model ${primaryModel} failed, trying fallback:`,
          error.message
        );
        try {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Create stunning color palettes with AI assistance. Generate
            harmonious color schemes that perfectly match your brand and design
            goals.
          </p>
        </motion.div>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8 sticky top-8">
              ...
              <form className="space-y-6">
                ...
                <p className="text-xs text-gray-600 text-center">
                  AI will analyze your preferences and generate harmonious color
                  combinations
                </p>
              </form>
            </div>
            ...
            {generatedPalettes.length === 0 ? (
              <div className="bg-white rounded-2xl shadow-xl p-12 text-center">
                ...
                <p className="text-gray-600 mb-6">
                  Fill out the form and click "Generate Palettes" to get
                        ...
                        <div className="mb-6">
                          <div className="rounded-lg p-4">
                            ...
                            {(() => {
                              ...
                              const textOnDark =
                                getContrastRatio(lightHex, bg.hex) >= 4.5
                                  ? lightHex
                                  : getContrastRatio(darkHex, bg.hex) >= 4.5
                                  ? darkHex
                                  : getContrastRatio(lightHex, bg.hex) >
                                    getContrastRatio(darkHex, bg.hex)
                                  ? lightHex
                                  : darkHex;

                              const chipColors = colors.slice(
                                0,
                                Math.min(3, colors.length)
                              );

                              const chipText = (hex) => {
                                return getContrastRatio("#FFFFFF", hex) >= 4.5
                                  ? "#FFFFFF"
                                  : "#111827";
                              };

                              const titleText = `${
                                palette.name || "Accessible Preview"
                              }`;
                              const bodyText = `Legible text preview on ${
                                bg.name || "background"
                              }.`;

                              return (
                                <div
                                  className="mt-4 mx-auto w-full max-w-2xl rounded-xl p-4 shadow-sm overflow-hidden"
                                  style={{
