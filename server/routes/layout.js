...
router.post(
  "/generate",
  auth,
  checkUsageLimit("layoutsGenerated"),
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      // Patch: get both the AI response and the raw Gemini response (if available)
      ...
      // If the AI service attached a rawGeminiResponse, include it in the API response

      // Create layout in database
      // Defensive output validation of AI response
      // Normalize components: AI may return strings or minimal objects; ensure { type, props } shape
      let safeComponents = [];
      if (Array.isArray(aiResponse.components)) {
        safeComponents = aiResponse.components
          .map((c, idx) => {
            // If component is a simple string, convert to { type: string, props: {} }
            if (typeof c === "string") {
              console.warn(
                `[Layout] Normalizing component at index ${idx} from string -> object:`,
                c
              );
              return { type: c.toLowerCase().replace(/\s+/g, "-"), props: {} };
            }

            // If the AI returned an object with a name/key but not the expected keys
            if (c && typeof c === "object") {
              const type =
                c.type || c.name || c.component || `component-${idx}`;
              const props = c.props || c.properties || c.attributes || {};
            ...
            console.warn(
              `[Layout] Dropping malformed component at index ${idx}:`,
              c
            );
            return null;
          })
      ...
      const layout = await Layout.create({
        ...
        aiModel: aiResponse.aiModel || "gemini-2.5-flash",
      });
      ...
      const responseData = {
        ...
        data: {
          ...
          rawGeminiResponse: aiResponse.rawGeminiResponse || null,
        },
      };
...
router.get(
  ...
  auth,
  asyncHandler(async (req, res) => {
    const layout = await Layout.findById(req.params.id).populate(
      "user",
  ...
  asyncHandler(async (req, res) => {
    ...
    const downloadData = {
      ...
      assets: layout.components
        .map((comp) => comp.properties?.assets)
        .filter(Boolean),
    };
