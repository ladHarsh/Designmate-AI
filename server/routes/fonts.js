...
router.post(
  "/suggest",
  auth,
  checkUsageLimit("fontSuggestions"),
  asyncHandler(async (req, res) => {
    ...
    console.log("📝 Font Suggestion Request:", {
      projectType,
      tone,
      accessibilityLevel,
    ...
    if (!projectType || !tone) {
      return res.status(400).json({
        success: false,
        message: "Project type and tone are required",
      });
    }
    ...
    try {
      ...
      res.json({
        ...
        data: aiResponse, // Send the complete AI response structure directly
      });
    } catch (error) {
