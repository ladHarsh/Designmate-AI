...
const ColorSchema = z.object({
  ...
  rgb: z.union([
    z
      .string()
      .regex(/^rgb\(\d+,\s*\d+,\s*\d+\)$/i, 'Invalid RGB format'),
    z.object({ r: z.number(), g: z.number(), b: z.number() })
  ]),
  hsl: z.union([
    z
      .string()
      .regex(/^hsl\(\d+,\s*\d+%,\s*\d+%\)$/i, 'Invalid HSL format'),
...
const PaletteSchema = z.object({
  ...
  accessibility: z.object({
    contrastRatio: z.preprocess((v) => typeof v === 'string' ? Number(v) : v, z.number().min(0)),
...
class ImprovedPalettePromptBuilder {
  ...
  validateOptions() {
    if (this.options.baseColor && !this.isValidHexColor(this.options.baseColor)) {
      throw new Error(`Invalid base color format: ${this.options.baseColor}`);
    }

    if (this.options.colorCount && (this.options.colorCount < 3 || this.options.colorCount > 20)) {
      throw new Error(`Color count must be between 3 and 20, got: ${this.options.colorCount}`);
    }

    // Validate against PALETTE_CONFIG
    const { mood, industry, paletteType, colorHarmony, accessibilityLevel } = this.options;
    if (!PALETTE_CONFIG.moods[mood]) throw new Error(`Invalid mood: ${mood}`);
    if (!PALETTE_CONFIG.industries[industry]) throw new Error(`Invalid industry: ${industry}`);
    if (!PALETTE_CONFIG.paletteTypes[paletteType]) throw new Error(`Invalid palette type: ${paletteType}`);
    if (!PALETTE_CONFIG.colorHarmonyTypes[colorHarmony]) throw new Error(`Invalid color harmony: ${colorHarmony}`);
    if (!['AA', 'AAA'].includes(accessibilityLevel)) throw new Error(`Invalid accessibility level: ${accessibilityLevel}`);
  }
  ...
  buildSystemPrompt() {
    ...
    const colorCountConstraint = this.options.colorCount 
      ...
      : (isSimpleFormat ? '4-6' : '8-15');

    return `You are a world-class color theory expert and UI/UX designer specializing in accessible color palette creation.

TASK: Generate a ${this.options.mood} color palette for ${this.getIndustryDescription()} with modern design principles.
...
5. ${isSimpleFormat ? 'Focus on core colors only' : 'Include comprehensive color system with states and variants'}`;
  }
  ...
  buildPaletteRequirements() {
    ...
    const keywordInstruction = this.options.keywords.length > 0
...
- Accessibility: Minimum ${this.options.accessibilityLevel === 'AAA' ? '7:1' : '4.5:1'} contrast ratio for text${baseColorInstruction}${keywordInstruction}`;
  }

  buildColorRequirements() {
    if (this.options.returnFormat === 'simple') {
      return `COLOR REQUIREMENTS:
- Generate exactly ${this.options.colorCount || 4} UNIQUE colors (no duplicate hex values)
    ...
    return `REQUIRED JSON STRUCTURE (ALL HEX VALUES MUST BE UNIQUE):
{
  ...
  "colors": {
    "primary": {
      "hex": "#${this.options.baseColor ? this.options.baseColor.slice(1) : '3B82F6'}",
      "rgb": "rgb(59,130,246)",
      "hsl": "hsl(217,91%,60%)",
      "name": "Primary Blue",
  ...
  "accessibility": {
    ...
    "colorBlindSafe": true,
    "notes": "All text-background combinations meet ${this.options.accessibilityLevel} standards"
  },
  "usage": {
...
- Notification and badge colors`);
  ...
  build() {
    const { mood, industry, paletteType, colorHarmony, accessibilityLevel, prompt, model } = this.options;
...
class PaletteValidator {
  ...
  static validatePaletteLogic(palette) {
    ...
    const hexValues = Object.values(colors).map(c => (c && c.hex ? String(c.hex).toLowerCase() : '')).filter(Boolean);
    const duplicates = hexValues.filter((hex, index) => hexValues.indexOf(hex) !== index);
    if (duplicates.length > 0) {
      warnings.push(`Duplicate colors found: ${[...new Set(duplicates)].join(', ')}`);
    }
    if (colors.text && colors.background) {
      const ratio = ColorUtils.getContrastRatio(colors.text.hex, colors.background.hex);
      const minRatio = accessibility.level === 'AAA' ? 7.0 : 4.5;
      if (ratio < minRatio) {
        errors.push(`Text-background contrast ratio ${ratio.toFixed(2)} is below minimum ${minRatio}`);
      }
    }
    ...
    for (const [key, color] of Object.entries(colors)) {
      if (!color || !color.hex || !/^#[0-9A-F]{6}$/i.test(color.hex)) {
        warnings.push(`Invalid or missing hex for ${key}: ${color && color.hex}`);
      }
    }
...
class PaletteNormalizer {
    ...
    try {
      ...
      if (Array.isArray(palette.colors)) {
        ...
        palette.colors.forEach((color, index) => {
          const key = color.name ? color.name.toLowerCase().replace(/\s+/g, '') : `color${index + 1}`;
          colorMap[key] = color;
        });
          ...
          if (color.hex) {
            try {
              ...
              if (typeof color.rgb === 'string') {
                const m = color.rgb.match(/rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/i);
                rgbObj = m ? { r: Number(m[1]), g: Number(m[2]), b: Number(m[3]) } : undefined;
              }
              
              if (typeof color.hsl === 'string') {
                const m = color.hsl.match(/hsl\(\s*(\d+)\s*,\s*(\d+)%\s*,\s*(\d+)%\s*\)/i);
                hslObj = m ? { h: Number(m[1]), s: Number(m[2]), l: Number(m[3]) } : undefined;
              }
              ...
              if (!hslObj) {
                ...
                hslObj = { h: Math.round(hsl.h), s: Math.round(hsl.s), l: Math.round(hsl.l) };
              }
      ...
      if (!palette.colors.background) {
        ...
        palette.colors.background = { hex: bgFmt.hex, rgb: { r: 255, g: 255, b: 255 }, hsl: { h: 0, s: 0, l: 100 }, name: 'Background', usage: 'Background' };
      }
      if (!palette.colors.text) {
        ...
        const tHslMatch = tFmt.hsl.match(/hsl\((\d+),\s*(\d+)%\s*,\s*(\d+)%\)/i);
        palette.colors.text = {
          hex: tFmt.hex,
          rgb: tRgbMatch ? { r: Number(tRgbMatch[1]), g: Number(tRgbMatch[2]), b: Number(tRgbMatch[3]) } : undefined,
          hsl: tHslMatch ? { h: Number(tHslMatch[1]), s: Number(tHslMatch[2]), l: Number(tHslMatch[3]) } : undefined,
          name: 'Text',
          usage: 'Primary text'
        };
      }
      ...
      if (!palette.accessibility) {
        ...
        if (!palette.accessibility.level || !['AA', 'AAA'].includes(palette.accessibility.level)) {
  ...
  static generateFallbackPalette(mood, industry) {
    return {
      ...
      colorHarmony: 'balanced',
      colors: {
        primary: { hex: '#3B82F6', rgb: { r: 59, g: 130, b: 246 }, hsl: { h: 217, s: 91, l: 60 }, name: 'Blue', usage: 'Primary' },
        background: { hex: '#FFFFFF', rgb: { r: 255, g: 255, b: 255 }, hsl: { h: 0, s: 0, l: 100 }, name: 'White', usage: 'Background' },
        text: { hex: '#111827', rgb: { r: 17, g: 24, b: 39 }, hsl: { h: 220, s: 39, l: 11 }, name: 'Dark', usage: 'Text' }
      },
      gradients: {},
      interactiveStates: {},
      shadows: {},
...
class ColorPaletteGenerator {
  ...
  async generate(options) {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        try {
          ...
          if (/overloaded|503|unavailable|busy|quota|429|rate limit|too many requests/i.test(message)) {
            const fallback = process.env.GEMINI_FALLBACK_MODEL || 'gemini-2.5-flash';
            console.warn(`[AI][Palette] Primary model failed (${model}) with error: ${message.substring(0, 100)}...`);
            console.warn(`[AI][Palette] Falling back to: ${fallback}`);
            try {
              result = await runWithModel(fallback);
            } catch (fallbackErr) {
              console.error(`[AI][Palette] Fallback model also failed:`, fallbackErr.message);
              throw fallbackErr;
            }
          } else {
            throw err;
        ...
        const normalizedPalette = PaletteNormalizer.normalize(rawPalette, mood, industry);
        const validation = PaletteValidator.validate(normalizedPalette);
        if (!validation.isValid) {
          console.warn(`[AI][Palette] Validation failed on attempt ${attempt}:`, validation.errors);
        ...
        if (validation.warnings.length > 0) {
          console.warn('[AI][Palette] Validation warnings:', validation.warnings);
        }
        return normalizedPalette;
      } catch (error) {
        lastError = error;
        console.error(`[AI][Palette] Attempt ${attempt} failed:`, error.message);
        if (attempt < this.maxRetries) {
    ...
    console.error('[AI][Palette] All attempts failed, returning fallback palette');
